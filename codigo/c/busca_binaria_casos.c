...
int main()
{
    ...
    fprintf(saida_est, "n,caso,media_ns,desvio_ns\n");
    fprintf(saida_brutos, "n,caso,run,tempo_ns\n");

    const char *casos[] = {"MC", "CM", "PC"}; // melhor, médio e pior caso
        ...
        for (int tipo = 0; tipo < 3; tipo++)
        {
            ...
            for (int run = 1; run <= RUNS; run++)
            {
                ...
                if (tipo == 0) // MC - melhor caso
                    ...
                    chave = (rand() % 2 == 0) ? vetor[rand() % n] : vetor[n - 1] + (rand() % 500);
                else                              // PC - pior caso
                    chave = vetor[n - 1] + 10000; // garante que não existe

                double tempo = medir_tempo_execucao_ns(vetor, n, chave);
                tempos[run - 1] = tempo;

                fprintf(saida_brutos, "%d,%s,%d,%.2f\n", n, casos[tipo], run, tempo);
                free(vetor);
            }
            ...
            fprintf(saida_est, "%d,%s,%.2f,%.2f\n", n, casos[tipo], media, desvio);
            printf("n=%d (%s) concluído (média=%.2f ns, desvio=%.2f ns)\n", n, casos[tipo], media, desvio);
        }
    }
